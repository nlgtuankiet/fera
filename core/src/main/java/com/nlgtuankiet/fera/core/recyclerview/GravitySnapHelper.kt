 ...
 *
 * @param onSnap A listener that's called when the [RecyclerView] used by [GravitySnapHelper]
...
class GravitySnapHelper @JvmOverloads constructor(
  gravity: Int,
  var enableSnapLastItem: Boolean = false,
  private var onSnap: ((Int) -> Unit)? = null
) : LinearSnapHelper() {
  ...
  private val notNullRecyclerView: RecyclerView
    get() = requireNotNull(recyclerView)
  ...
  override fun calculateDistanceToFinalSnap(
    ...
    if (gravity == Gravity.CENTER) {
      return requireNotNull(super.calculateDistanceToFinalSnap(layoutManager, targetView))
    }
    val out = IntArray(2)
  ...
  override fun calculateScrollDistance(velocityX: Int, velocityY: Int): IntArray {
    ...
    val scroller = Scroller(
      requireNotNull(recyclerView).context,
   ...
   * and snap according to [GravitySnapHelper.enableSnapLastItem],
  ...
  fun updateSnap(smooth: Boolean, checkEdgeOfList: Boolean) {
    ...
    val lm = requireNotNull(notNullRecyclerView.layoutManager)
  ...
  fun getCurrentSnappedPosition(): Int {
    if (recyclerView != null && recyclerView?.layoutManager != null) {
      val snappedView = findSnapView(requireNotNull(recyclerView?.layoutManager))
      if (snappedView != null) {
        return requireNotNull(recyclerView).getChildAdapterPosition(snappedView)
      }
    }
  ...
  private fun getFlingDistance(): Int {
    return when {
      maxFlingSizeFraction != FLING_SIZE_FRACTION_DISABLE -> {
        when {
          verticalHelper != null -> (notNullRecyclerView.height * maxFlingSizeFraction).toInt()
          horizontalHelper != null -> (notNullRecyclerView.width * maxFlingSizeFraction).toInt()
          else -> Int.MAX_VALUE
        }
      }
  ...
  private fun scrollTo(position: Int, smooth: Boolean): Boolean {
    if (recyclerView?.layoutManager != null) {
      if (smooth) {
        val smoothScroller = createScroller(requireNotNull(recyclerView?.layoutManager))
  ...
  private fun findView(
    ...
    if (checkEdgeOfList && isAtEdgeOfList(layoutManager) && !enableSnapLastItem) {
      return null
    }
  ...
  /**
   * Dispatches a [onSnap] event if the snapped position
  ...
  private fun onScrollStateChanged(newState: Int) {
    if (newState == RecyclerView.SCROLL_STATE_IDLE && onSnap != null) {
      if (isScrolling) {
        if (nextSnapPosition != RecyclerView.NO_POSITION) {
          onSnap?.invoke(nextSnapPosition)
        } else {
   ...
   *
   *
   * This makes sure that a position is reported in [onSnap]
   */
  private fun dispatchSnapChangeWhenPositionIsUnknown() {
    val layoutManager = recyclerView?.layoutManager ?: return
    val snapView = findSnapView(layoutManager, false) ?: return
    val snapPosition = notNullRecyclerView.getChildAdapterPosition(snapView)
    if (snapPosition != RecyclerView.NO_POSITION) {
      onSnap?.invoke(snapPosition)
    }
  }
